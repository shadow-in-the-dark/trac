...
def extract_cities_from_trip(text: str) -> list[str]:
    ...
    pattern = re.compile(r"([A-Za-z][a-zA-Z\s\.\-]{2,30}),\s*([A-Z]{2})(?:\s+\d{5}(?:-\d{4})?)?", re.MULTILINE)
    skip = {"Loaded", "Drop", "Preloaded", "Route", "Ave", "Blvd", "St", "Dr", "Tue", "Wed", "Thu", "Fri", "Mon", "Sat", "Sun", "Apr", "May", "Jun", "Jul"}
    cities, seen = [], set()
