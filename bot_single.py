...
def extract_cities_regex(text: str) -> list[str]:
    ...
    for city, state in matches:
        city = city.strip()
        # Убираем мусор — слишком короткие
        if len(city) < 3:
            continue
        # Убираем коды складов Amazon: ONT5, EWR8, LAX9 и т.д.
        if re.match(r"^[A-Z]{2,4}\d+$", city):
            continue
        # Убираем известные не-города
        skip = {"Loaded", "Drop", "Preloaded", "Route", "Ave", "Blvd", "St", "Dr"}
        if city in skip:
            continue
        key = f"{city}, {state}"
        if key not in seen:
...
async def cmd_routeweather(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Начало диалога: запрос точки А. Доступно всем в группе."""
